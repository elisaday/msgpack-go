...
func TestUnpackUInt64(t *testing.T) {
	...
	v := []uint64{0, 1, 31, 32, 33, 63, 64, 65, 126, 127, 128, 129,
		192, 201, 254, 255, 256,
		32764, 32765, 32766, 65534, 65535, 65536,
		2147483646, 2147483647, 21474836478,
		4294967294, 4294967295, 4294967296,
...
func TestPackUInt32(t *testing.T) {
	...
	for _, i := range []uint32{0, 1, 31, 32, 33, 63, 64, 65, 126, 127, 128, 129,
		192, 201, 254, 255, 256,
		32764, 32765, 32766, 65534, 65535, 65536,
	...
	if bytes.Compare(b.Bytes(), []byte{
		...
		0x0, 0xcd, 0x7f, 0xfc, 0xcd, 0x7f, 0xfd, 0xcd, 0x7f, 0xfe, 0xcd, 0xff,
		0xfe, 0xcd, 0xff, 0xff, 0xce, 0x0, 0x1, 0x0, 0x0, 0xce, 0x7f, 0xff,
		0xff, 0xfe, 0xce, 0x7f, 0xff, 0xff, 0xff, 0xce, 0xff, 0xff, 0xff, 0xfe,
		0xce, 0xff, 0xff, 0xff, 0xff}) != 0 {
		t.Error("wrong output", b.Bytes())
	}
}
...
func TestUnpackUInt32(t *testing.T) {
	...
	v := []uint32{0, 1, 31, 32, 33, 63, 64, 65, 126, 127, 128, 129,
		192, 201, 254, 255, 256,
		32764, 32765, 32766, 65534, 65535, 65536,
		2147483646, 2147483647,
		4294967294, 4294967295}
...
func TestPackInt64(t *testing.T) {
	...
	if bytes.Compare(b.Bytes(), []byte{
		...
		0xff, 0xff, 0xff,
		0x0, 0xff, 0xe1, 0xe0, 0xd0, 0xdf, 0xd0, 0xc1, 0xd0, 0xc0, 0xd0,
...
func TestUnpackInt64(t *testing.T) {
	...
	v := []int64{0, 1, 31, 32, 33, 63, 64, 65, 126, 127, 128, 129,
		192, 201, 254, 255, 256,
		32764, 32765, 32766, 65534, 65535, 65536,
		2147483646, 2147483647, 21474836478,
		4294967294, 4294967295, 4294967296,
	...
	for i := 0; i < len(v); i++ {
		val, err := UnpackInt64(b, &offset)
		t.Logf("%d %d %d", val, v[i], offset)
		if err != nil || val != v[i] {
			t.Errorf("wrong output")
		}
	}
...
func TestPackInt32(t *testing.T) {
	...
	for _, i := range []int32{0, 1, 31, 32, 33, 63, 64, 65, 126, 127, 128, 129,
		192, 201, 254, 255, 256,
		32764, 32765, 32766, 65534, 65535, 65536,
		2147483646, 2147483647,
		0, -1, -31, -32, -33, -63, -64, -65, -126, -127, -128, -129,
	...
	if bytes.Compare(b.Bytes(), []byte{
		...
		0xd1, 0x1, 0x0, 0xd1, 0x7f, 0xfc, 0xd1, 0x7f, 0xfd, 0xd1, 0x7f, 0xfe, 0xd2, 0x0,
		0x0, 0xff, 0xfe, 0xd2, 0x0, 0x0, 0xff, 0xff, 0xd2, 0x0, 0x1, 0x0, 0x0, 0xd2, 0x7f,
		0xff, 0xff, 0xfe, 0xd2, 0x7f, 0xff, 0xff, 0xff,
		0x0, 0xff, 0xe1, 0xe0, 0xd0, 0xdf, 0xd0, 0xc1, 0xd0, 0xc0, 0xd0,
		0xbf, 0xd0, 0x82, 0xd0, 0x81, 0xd0, 0x80, 0xd1, 0xff, 0x7f, 0xd1, 0xff, 0x40, 0xd1,
		0xff, 0x37, 0xd1, 0xff, 0x2, 0xd1, 0xff, 0x1, 0xd1, 0xff, 0x0, 0xd1, 0x80, 0x4, 0xd1,
		0x80, 0x3, 0xd1, 0x80, 0x2, 0xd2, 0xff, 0xff, 0x0, 0x2, 0xd2, 0xff, 0xff, 0x0, 0x1,
		0xd2, 0xff, 0xff, 0x0, 0x0, 0xd2, 0x80, 0x0, 0x0, 0x2, 0xd2, 0x80, 0x0, 0x0, 0x1}) != 0 {
		t.Error("wrong output", b.Bytes())
	}
}
...
func TestUnpackInt32(t *testing.T) {
	...
	v := []int32{0, 1, 31, 32, 33, 63, 64, 65, 126, 127, 128, 129,
		192, 201, 254, 255, 256,
		32764, 32765, 32766, 65534, 65535, 65536,
		2147483646, 2147483647,
		0, -1, -31, -32, -33, -63, -64, -65, -126, -127, -128, -129,
...
func TestUnpackFloat32(t *testing.T) {
	b := []byte{
		0xca, 0x43, 0xa2, 0x1d, 0xf4,
		0xca, 0x41, 0xb8, 0xfb, 0xe7,
		0xca, 0x40, 0x49, 0xf, 0xda}

	v := []float32{324.234, 23.123, 3.1415926}

	offset := uint32(0)

	for i := 0; i < len(v); i++ {
		val, err := UnpackFloat(b, &offset)
		t.Logf("%f %f %d", val, v[i], offset)
		if err != nil || val != v[i] {
			t.Errorf("wrong output")
		}
	}
...
func TestUnpackFloat64(t *testing.T) {
	b := []byte{
		0xcb, 0x40, 0x74, 0x43, 0xb3, 0x45, 0xe7, 0x74, 0x7d,
		0xcb, 0x40, 0x37, 0x2d, 0x47, 0x36, 0x6c, 0x3, 0x56,
		0xcb, 0x40, 0x9, 0x22, 0x28, 0x6e, 0x58, 0xc4, 0x5}

	v := []float64{324.23126783764, 23.176867867823, 3.1416786785926}

	offset := uint32(0)

	for i := 0; i < len(v); i++ {
		val, err := UnpackDouble(b, &offset)
		t.Logf("%f %f %d", val, v[i], offset)
		if err != nil || val != v[i] {
...
func TestUnpackRawBuffer(t *testing.T) {
	b := []byte{0xab, 0x68, 0x65, 0x6c, 0x6c, 0x6f, 0x20, 0x77, 0x6f, 0x72, 0x6c, 0x64, 0xda, 0x2,
		0x8, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6a, 0x6b, 0x6c, 0x6d,
		0x6e, 0x6f, 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7a, 0x61,
		0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6a, 0x6b, 0x6c, 0x6d, 0x6e, 0x6f,
		0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7a, 0x61, 0x62, 0x63,
	...
	for i := 0; i < len(v); i++ {
		val, err := UnpackRawBuffer(b, &offset)
		t.Logf("%s %s", string(val), string(v[i]))
		if err != nil || bytes.Compare(val, v[i]) != 0 {
			t.Error("wrong output")
		}
	}
}
